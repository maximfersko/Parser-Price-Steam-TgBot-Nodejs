...
import { getInventoryValue } from "../services/steam.js";

const welcomeMessage = (ctx) => `Добро пожаловать ${ctx.from.username} ! Выбери один из пунктов.`;
...
const getMainMenuKeyboard = () => {
    return Markup.inlineKeyboard([
        ...
        [Markup.button.callback(COMMAND_TEXT.setOperation, DATA_ACTION.op)]
    ]);
}
...
export const backToMainMenuCallback = (ctx) => {
    cleanInlKeyboard(ctx);
    ctx.editMessageText(welcomeMessage(ctx), getMainMenuKeyboard());
}
